...
namespace ScoreSaber.Menu.Multiplayer;
...
internal class ScoreSaberMultiplayerLevelSelectionLeaderboardFlowManager : IInitializable, IDisposable {
    ...
    private void HideLeaderboard() {
        ...
        if (_platformLeaderboardViewController.isInViewControllerHierarchy) {
            ...
            if (currentFlowCoordinator is not MultiplayerLevelSelectionFlowCoordinator)
                return;

            currentFlowCoordinator.InvokeMethod<object, FlowCoordinator>("SetRightScreenViewController", null, ViewController.AnimationType.Out);
        }
    }
    ...
    private void ShowLeaderboard() {
        ...
        var currentFlowCoordinator = _mainFlowCoordinator.YoungestChildFlowCoordinatorOrSelf();
        currentFlowCoordinator.InvokeMethod<object, FlowCoordinator>("SetRightScreenViewController", _platformLeaderboardViewController, ViewController.AnimationType.In);
        ...
        if (_performingFirstActivation) {
            ...
            _ = Task.Run(async () => {
                ...
                _ = UnityMainThreadTaskScheduler.Factory.StartNew(() => _platformLeaderboardViewController.Refresh(true, true));
            });
        }
    }
